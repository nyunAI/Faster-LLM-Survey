...
from transformers.utils.quantization_config import AWQLinearVersion, ExllamaVersion
...
def get_parser():
    ...
    parser.add_argument(
        "--awq",
        action="store_true",
        help="Indicate that the model to benchmark is a AWQ model.",
    )
    parser.add_argument(
        "--bitsandbytes",
        action="store_true",
    ...
    parser.add_argument(
        "--awq-version",
        type=str,
        choices=[v.name.lower() for v in (AWQLinearVersion.GEMM, AWQLinearVersion.GEMV)],
        default="gemm",
        help='AWQ version "gemm" or "gemv".',
    )
    parser.add_argument(
        "--sweep",
        action="store_true",
...
if args.gptq:
    quantization_config = GPTQConfig(
        bits=args.bits, use_exllama=args.use_exllama, exllama_config={"version": args.exllama_version}
    )
    ...
    quantization_config = AwqConfig(
        bits=args.bits, group_size=128, zero_point=True, version=args.awq_version
    )
    model = autoclass.from_pretrained(
        args.model, quantization_config=quantization_config, device_map="auto", torch_dtype=torch.float16
    )
...
print(f"Model uses AWQ: {uses_awq}")
