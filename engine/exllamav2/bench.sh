...
sleep 3
python $METHOD_PATH/test_inference.py -ed $DATASET_PATH -gs 40 -m $MODEL_PATH -s 2>&1 | tee $METHOD_EXPORTS/bench_baseline.log
killall nvidia-smi
...
MODEL_2BPW_EXL2="2bpw-exl2"

# Loop through each model type
for MODEL_TYPE in "$MODEL_4BPW_EXL2" "$MODEL_8BPW_EXL2" "$MODEL_2BPW_EXL2"; do
    ...
    python $METHOD_PATH/test_inference.py -ed $DATASET_PATH -gs 40 -m $MODEL_PATH -s 2>&1 | tee $METHOD_EXPORTS/bench_$MODEL_TYPE.log
...
#     # Run llama-bench for the current model type
#     python $METHOD_PATH/test_inference.py -ed $DATASET_PATH -gs 40 -m $MODEL_PATH -p "What is the meaning of life?" 2>&1 | tee $METHOD_EXPORTS/bench_prompt_$MODEL_TYPE.log
